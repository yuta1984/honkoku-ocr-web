...
| OCRエンコーダ | `kuzushiji-v7-encoder-int8.onnx`（ConvNeXt+2D位置埋込） | pixel_values[1,3,128,1024] → encoder_hidden_states[1,128,D] |
| OCRデコーダ | `kuzushiji-v7-decoder-int8.onnx`（RoBERTa, greedy, KVキャッシュ無し） | input_ids[1,T]+encoder_hidden_states → logits。CLS=2 から SEP=3 まで逐次生成 |
